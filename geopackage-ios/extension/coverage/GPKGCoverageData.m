...
-(instancetype) initWithGeoPackage: (GPKGGeoPackage *) geoPackage andTileDao: (GPKGTileDao *) tileDao andWidth: (NSNumber *) width andHeight: (NSNumber *) height andProjection: (PROJProjection *) requestProjection{
    ...
    if(self != nil){
        ...
        self.coverageProjection = [[geoPackage tileMatrixSetDao] projection:tileDao.tileMatrixSet];
        self.coverageBoundingBox = [tileDao.tileMatrixSet boundingBox];
...
-(GPKGGriddedCoverage *) queryGriddedCoverage{
    if([self.griddedCoverageDao tableExists]){
        _griddedCoverage = [self.griddedCoverageDao queryByTileMatrixSet:self.tileMatrixSet];
    }
    return _griddedCoverage;
}
...
-(GPKGGriddedTile *) griddedTileWithResultSet: (GPKGResultSet *) resultSet{
    return (GPKGGriddedTile *)[self.griddedTileDao object:resultSet];
}
 ...
 */
-(NSArray *) valuesFromDictionary: (NSDictionary *) dictionary atRow: (int) row andColumn: (int) column{
 ...
 * @return coverage data values
 */
-(NSArray *) valuesFromCoverageDataDictionary: (GPKGCoverageDictionary *) coverageDataDictionary atRow: (int) row andColumn: (int) column{
...
-(NSArray *) formatUnboundedResultsWithTileMatrix: (GPKGTileMatrix *) tileMatrix andCoverageDictionary: (GPKGCoverageDictionary *) coverageDictionary andTileCount: (int) tileCount andMinRow: (int) minRow andMaxRow: (int) maxRow andMinColumn: (int) minColumn andMaxColumn: (int) maxColumn{
    ...
    if(coverageDictionary.dictionary.count != 0){
        ...
        if (tileCount == 1) {
            values = (NSMutableArray *)[self valuesFromCoverageDataDictionary:coverageDictionary atRow:minRow andColumn:minColumn];
        } else {
...
-(NSArray *) nearestNeighborsWithXSource: (float) xSource andYSource: (float) ySource {
    ...
    [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:firstX], [NSNumber numberWithInt:firstY], nil]];
    
    // Add the second and third neighbor based upon the x and y distances to
    // second coordinates
    if (xDistance <= yDistance) {
        [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:secondX], [NSNumber numberWithInt:firstY], nil]];
        [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:firstX], [NSNumber numberWithInt:secondY], nil]];
    } else {
        [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:firstX], [NSNumber numberWithInt:secondY], nil]];
        [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:secondX], [NSNumber numberWithInt:firstY], nil]];
    }
    ...
    [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:secondX], [NSNumber numberWithInt:secondY], nil]];
    
    // If right on the boundary between the forward and backwards pixel, add
    // the backwards pixel options
    if (xPixel.offset == 0) {
        [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:xPixel.min - 1], [NSNumber numberWithInt:yPixel.min], nil]];
        [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:xPixel.min - 1], [NSNumber numberWithInt:yPixel.max], nil]];
    }
    if (yPixel.offset == 0) {
        [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:xPixel.min], [NSNumber numberWithInt:yPixel.min - 1], nil]];
        [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:xPixel.max], [NSNumber numberWithInt:yPixel.min - 1], nil]];
    }
    if (xPixel.offset == 0 && yPixel.offset == 0) {
        [results addObject:[NSArray arrayWithObjects:[NSNumber numberWithInt:xPixel.min - 1], [NSNumber numberWithInt:yPixel.min - 1], nil]];
    }
...
-(GPKGCoverageDataSourcePixel *) minAndMaxOfXSource: (float) source{
    ...
    float valueLocation = [self encodedLocationWithX:floor andEncoding:[self.griddedCoverage gridCellEncodingType]];
...
-(GPKGCoverageDataSourcePixel *) minAndMaxOfYSource: (float) source{
    ...
    int floor = (int) floorf(source);
    float valueLocation = [self encodedLocationWithY:floor andEncoding:[self.griddedCoverage gridCellEncodingType]];
...
-(NSArray *) valuesWithTileMatrix: (GPKGTileMatrix *) tileMatrix andTileResults: (GPKGResultSet *) tileResults andRequest: (GPKGCoverageDataRequest *) request andTileWidth: (int) tileWidth andTileHeight: (int) tileHeight andOverlappingPixels: (int) overlappingPixels{
    ...
    while ([tileResults moveToNext]) {
        ...
        if (currentRow > previousRow) {
            ...
            lastRowsByColumn = [NSMutableDictionary dictionary];
            leftLastColumns = nil;
        }
        ...
        if (overlap != nil) {
            ...
            CGRect src = [GPKGTileBoundingBoxUtils rectangleWithWidth:[tileMatrix.tileWidth intValue] andHeight:[tileMatrix.tileHeight intValue] andBoundingBox:tileBoundingBox andSection:overlap];
            ...
            if ([request.projectedBoundingBox equals:overlap]) {
                ...
                dest = [GPKGTileBoundingBoxUtils rectangleWithWidth:tileWidth andHeight:tileHeight andBoundingBox:request.projectedBoundingBox andSection:overlap];
            }
    ...
    while([tileResults moveToNext]){
        ...
        if (overlap != nil) {
            ...
            if (src.size.width >= 0 && src.size.height >= 0) {
                ...
                [columnsMap setObject:values forKey:[NSNumber numberWithInt:[tileRow tileColumn]]];
                
                // Increase the contributing tiles count
                tileCount++;
                
                // Track the min and max row and column
                minRow = [NSNumber numberWithInt:(minRow == nil ? [tileRow tileRow] : MIN([minRow intValue], [tileRow tileRow]))];
                maxRow = [NSNumber numberWithInt:(maxRow == nil ? [tileRow tileRow] : MAX([maxRow intValue], [tileRow tileRow]))];
                minColumn = [NSNumber numberWithInt:(minColumn == nil ? [tileRow tileColumn] : MIN([minColumn intValue], [tileRow tileColumn]))];
                maxColumn = [NSNumber numberWithInt:(maxColumn == nil ? [tileRow tileColumn] : MAX([maxColumn intValue], [tileRow tileColumn]))];
            }
        }
    }
...
-(GPKGTileMatrix *) tileMatrixWithRequest: (GPKGCoverageDataRequest *) request{
    ...
    if([request overlapWithBoundingBox:self.coverageBoundingBox] != nil){
        ...
        if (zoomLevel != nil) {
            tileMatrix = [self.tileDao tileMatrixWithZoomLevel:[zoomLevel intValue]];
        }
    }
